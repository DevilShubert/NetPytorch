...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs):
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            positive = torch.where(torch.ge(matched_idxs_per_image, 1))[0]  # 也就是实际anchors与GTBox匹配上的正样本数目
            # = 0的为负样本
            # negative = torch.nonzero(matched_idxs_per_image == 0).squeeze(1)
            negative = torch.where(torch.eq(matched_idxs_per_image, 0))[0]
            # 也就是实际anchors与GTBox的IoU小于0.3的数目 和 丢弃的anchors数目

            # 指定正样本的数量
            num_pos = int(self.batch_size_per_image * self.positive_fraction)
            # 256 * 0.5 = 128 或这是 512 * 0.25
            # protect against not enough positive examples
            # 如果正样本数量不够就直接采用所有正样本
            num_pos = min(positive.numel(), num_pos)
            # 指定负样本数量
            num_neg = self.batch_size_per_image - num_pos  # 256 - anchors与GTBox匹配上的正样本数目
            # protect against not enough negative examples
            # 如果负样本数量不够就直接采用所有负样本，在一般情况肯定是num_neg，因为图中的负样本要远多余num_neg
...
class BoxCoder(object):
    ...
    # 这里返回的值，就是相对于GTBox相差的四个参数数值（所有anchor），用于计算loss
    ...
    def decode(self, rel_codes, boxes):
        ...
        # boxes表示每张图片的anchors
        # rel_codes表示回归参数的信息
        # 下一步需要将rel_codes上的信息用在anchors上，得到预测的bbox，然后再和GTbox比较
        concat_boxes = torch.cat(boxes, dim=0)

        box_sum = 0  # 居然拼接一个batch当中所有图片的anchors数目
        for val in boxes_per_image:
            box_sum += val

        # 重点！：将预测的bbox回归参数应用到对应anchors上得到预测bbox的坐标
