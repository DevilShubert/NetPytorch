...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)):
        ...
        # 用于区别mobileNet网络的sizes和resNet网络的sizes
        ...
        self.cell_anchors = None  # anchor模版
        self._cache = {}
    ...
    # 返回anchors信息：有几个预测特征层，每个预测特征层上的有多少个anchors，每个anchor映射到原图的的位置信息
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 计算特征层上的一步等于原始图像上的步长（变换尺度）
        # 这里的变换尺度有两个用：一个是确定特征图上的像素点对应在原图上的像素点位置，
        # 另外一个是确定每一个像素点对应的15个anchor在原图上的长宽大小（变换尺度 * (32,), (64,), (128,), (256,), (512,)）
        strides = [[torch.tensor(image_size[0] // g[0], dtype=torch.int64, device=device),
                    torch.tensor(image_size[1] // g[1], dtype=torch.int64, device=device)] for g in grid_sizes]

        # 根据提供的sizes和aspect_ratios生成anchors模板，因为每一个像素对应到原图上都要有15个anchors
        self.set_cell_anchors(dtype, device)

        # 计算/读取所有anchors的坐标信息（这里的anchors信息是映射到原图上的所有anchors信息，不是anchors模板）
        # 得到的是一个list列表，对应每张预测特征图映射回原图的anchors坐标信息（所有预测特征层）
        ...
        # （简单理解就是由一张图片生成5个featureMap，5个featureMap又有5组anchors，现在将这5组anchors合在一起）
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 重点理解：直接用卷积操作，将channels变为所有anchors的二分类预测值
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 计算预测的目标bbox regression参数
        # 重点理解：直接用卷积操作，将channels变为所有anchors的回归参数
    ...
    def forward(self, x):
        ...
        logits = []
        bbox_reg = []
        # 此时共有5个预测特征层 5个featureMaps
        # 对应多个不同的预测特征层分别遍历生成：目标分数+边界框回归参数
        for i, feature in enumerate(x):
            t = F.relu(self.conv(feature))
            logits.append(self.cls_logits(t))  # 目标分数预测器
            bbox_reg.append(self.bbox_pred(t))  # 边界框预测器
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    的tensor排列顺序以及shape进行调整 -> [N, -1, C] ， N代表batch层
    Args:
    ...
    # 注意这里的展平处理：将batch和anchors维度叠在一起了
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        """
        通俗理解：可以认为这里开始筛选anchor boxes与GTBOXS的重叠面积很大的和很小 ，不大不小的剔除
        计算每个anchors最匹配的gt，并划分为正样本，背景以及废弃的样本
        Args：
            anchors: (List[Tensor])
            targets: (List[Dict[Tensor])
        Returns:
            labels: 标记anchors归属类别（1, 0, -1分别对应 正样本（对于0.7），背景（小于0.3），废弃的样本（0.7到0.3））
                    注意，在RPN中只有前景和背景，所有正样本的类别都是1，0代表背景
            matched_gt_boxes：与anchors匹配的gt
        """
        labels = []
        matched_gt_boxes = []
        # 遍历每张图像的anchors和targets，此时的anchors数目是一个batch中2张图片的所有
        # featureMaps上的所有anchors
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                # 设置下限为0，不要-2和-1的丢弃样本和负样本

                # 记录所有anchors匹配后的标签(正样本处标记为1，负样本处标记为0，丢弃样本处标记为-2)
                labels_per_image = matched_idxs >= 0
                labels_per_image = labels_per_image.to(dtype=torch.float32)

                # background (negative examples) 只提取负样本，即为-1的部分，其余都设置为0
                bg_indices = matched_idxs == self.proposal_matcher.BELOW_LOW_THRESHOLD  # -1
                labels_per_image[bg_indices] = 0.0

                # discard indices that are between thresholds 只提取丢弃的模版，即为-2的部分，设置为-1
    ...
    # 返回的labels的长度等于anchors数量，包含1 -1 -2三个分别表示正样本（0.7或最大anchor）,废弃anchor，负样本
    # 返回的matched_gt_boxes表示所有anchors所对应的GTBox
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        # idx预测特征层的索引，n为该预测特征层anchors的个数
        ...
        # 0、获取 每张预测特征图上 预测概率排前pre_nms_top_n的anchors索引值，也就是得到排名之后的索引值
        ...
        levels = levels[batch_idx, top_n_idx]  # levels就是原始排名和前2000个得分最高的的排名
        # 预测概率排前pre_nms_top_n的anchors索引值获取相应bbox坐标信息
        # 信息有两个：bbox位置信息 + 为目标的概率信息
        ...
        for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, image_shapes):
            '''
            proposals：bbox位置信息
            objectness_prob：为目标的概率信息
            levels：原始排名和得分最高的的排名 levels = levels[batch_idx, top_n_idx]
            image_shapes：原始图片尺寸大小
            '''
            # 调整预测的boxes信息，将越界的坐标调整到图片边界上
            # 将box限制在图像内部，img_shape表示图像高度和宽度内
            boxes = box_ops.clip_boxes_to_image(boxes, img_shape)

            # 1、返回boxes满足宽，高都大于min_size的索引
            keep = box_ops.remove_small_boxes(boxes, self.min_size)  # 删除小目标的boxes
            boxes, scores, lvl = boxes[keep], scores[keep], lvl[keep]

            # 2、移除小概率boxes，参考下面这个链接
            ...
            # keep为执行nmx排序之后，按照目标类别分数排序之后的bbox和bbox信息

            # keep only topk scoring predictions
            keep = keep[: self.post_nms_top_n()]  # 只需要获取前面2k个/所需个数的bbox（训练）
            # 这里应该是如果
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        # 注意这里的sampled_pos_inds正样本是一个batch中所有图像匹配到的正样本
    ...
    def forward(self,
        ...
        num_anchors_per_level = [s[0] * s[1] * s[2] for s in num_anchors_per_level_shape_tensors]  # 每个预测特征层anchor的个数

        # 调整内部tensor格式以及shape
        # 为什么要调整tensor的shape：因为后续的proposal filter和在train中的compute_loss更加方便
        ...
        # 这里的的proposals代表将regression参数应用到anchors上，在原图上的坐标位置信息：四个角的坐标
        ...
        losses = {}
        # 接下来只有训练才需要用到target信息用于和GTBOX比对
        # 注意target是在使用RPN的forward的时候才单独传入的，具体看faster_rcnn_framework.py的98行
        if self.training:
            assert targets is not None
            # 计算每个anchors最匹配的gt，并将anchors进行分类，前景，背景以及废弃的anchors
            labels, matched_gt_boxes = self.assign_targets_to_anchors(anchors, targets)
            # matched_gt_boxes包含我们的256个正负样本
            # 一种极端情况：我们的所有anchors居然没有一个和targets中的GTBox有相交的部分，其实这种情况是不太可能的因为anchors的数量很多，肯定会有覆盖
            regression_targets = self.box_coder.encode(matched_gt_boxes, anchors)
            # 返回所有分好类的所有的anchors对应的GTBox的四个参数，用于计算loss

            loss_objectness, loss_rpn_box_reg = self.compute_loss(
                # 预测的目标分类、逻辑回归器得到的目标偏移量、anchors对应真实GTBox类别、所有anchors对应GTBox的四个参数
