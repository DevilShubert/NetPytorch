...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        """
        将图片缩放到指定的大小范围内，并对应缩放bboxes信息（bboxes信息：从xml文件得到的图片信息，训练验证都有）
        Args:
            image: 输入的图片
        ...
        bbox = target["boxes"]
        # 根据图像的缩放比例来缩放bbox
        # 这一步仅仅是为了train数据的bbox要缩放，如果是test时则不需要，因为就没有bbox信息
    ...
    def postprocess(self,
        ...
        # result网络预测的bbox信息和类别信息
        # image_shapes图像resize之后的每一个图像的高宽
        # original_image_sizes顾名思义
        """
        ...
        return result  # 返回对应到原始图像的bbox信息
    ...
    def forward(self,
        ...
        # 记录resize后的图像尺寸
        image_sizes = [img.shape[-2:] for img in images]   # 此时还是打包之前，每张图片的长宽都可能不一样
        ...
        image_list = ImageList(images, image_sizes_list)  # images已经打包好了的维度是4的一组batch图像，
        # image_sizes_list是resize但是还没有填充打包的一组图像信息
